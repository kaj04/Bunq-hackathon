...
export const Dashboard: React.FC<DashboardProps> = ({ balance, transactions, requests, onAcceptRequest, onAddExpense }) => (
  <div className="p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 ml-64 max-w-[1240px]">
    ...
    <div className="flex justify-between items-start gap-8">
      ...
      <div className="flex gap-4">
        ...
        <button className="h-[160px] w-36 bg-card border border-zinc-800 rounded-[24px] flex flex-col items-center justify-center gap-3 hover:bg-zinc-800/50 transition-all active:scale-95 group">
          <div className="p-3 bg-white/5 rounded-2xl group-hover:bg-white/10 transition-colors">
            <ArrowDownLeft className="w-8 h-8 text-zinc-400" />
          </div>
          <span className="text-sm font-bold text-zinc-300 tracking-tight">Request</span>
        </button>
      </div>
    </div>
